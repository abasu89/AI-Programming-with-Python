...
def main(input, checkpoint):
    ...
    probs, classes = predict(input, model, in_arg.top_k)
...
def process_image(image):
    ...
    print ("Resized image size: {}".format(img.size))
    ...
    img = img.crop(area)
    print ('Cropped image size: {}'.format(img.size))
...
def predict(image_path, model, topk=5):
    ...
    print ('Image size: {}'.format(imgT.shape))
    ...
    print ('index to class dict: {}'.format(idx_to_class))
    classes = []
    for i in indices[0]:
...
def get_input_args():
    ...
    parser.add_argument('--gpu', type=None, help = 'Enter GPU mode') # DETERMINE TYPE
...
if __name__ == "__main__":
    main('flowers/train/100/image_07907.jpg', 'checkpoint.pth')